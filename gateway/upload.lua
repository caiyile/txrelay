...
local function http_post_client(url, data, timeout)
...
if ret_save then
    ngx.say("上传成功")
	local data = "{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", \"method\": \"downloadnote\", \"params\": [\"filename\"] }"
	local newData = string.gsub(data,'filename',name)
	local resp, err = http_post_client("http://120.77.45.167:6666/",newData,3000)
end